...
// define tables
  ...
  def matche = foreignKey("MP_MATCH_FK", matchId, TableQuery[Matches])(_.id)

  def player = foreignKey("MP_PLAYER_FK", playerId, TableQuery[Players])(_.id)

  def team = foreignKey("MP_TEAM_FK", teamId, TableQuery[Teams])(_.id)
